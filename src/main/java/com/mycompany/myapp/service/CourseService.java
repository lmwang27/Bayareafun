...
public class CourseService {
    ...
    public List<CourseDto> findAllCoursesFromDBLengthLargerThan10(){
        List<CourseDto> list = courseRepository.findAllCoursesDto();
        List<CourseDto> res = new ArrayList<>();
        if(list.size()>0){
            for(CourseDto tt: list){
                if(tt.getCourseName().length()>10){
                    res.add(tt);
                }
            }
        }
